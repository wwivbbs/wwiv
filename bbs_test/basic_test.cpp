...
class TestMacroContext : public MacroContext {
public:
  ...
  [[nodiscard]] const directoryrec& dir() const override { return dir_; }
...
protected:
  ...
  void RegisterBasicUnitTestModule(mb_interpreter_t* bas) {
    ...
    mb_register_func(bas, "EQUALS", [](struct mb_interpreter_t* bas, void** l) -> int {
      ...
      if (mb_has_arg(bas, l)) {
        mb_check(mb_pop_string(bas, l, &msg));
      }
      ...
      switch (v1.type) {
      case MB_DT_INT:
        return v1.value.integer == v2.value.integer ? MB_FUNC_OK : MB_FUNC_ERR;
      case MB_DT_REAL:
        return v1.value.float_point == v2.value.float_point ? MB_FUNC_OK : MB_FUNC_ERR;
      case MB_DT_STRING:
        return IsEquals(v1.value.string, v2.value.string) ? MB_FUNC_OK : MB_FUNC_ERR;
      default:
