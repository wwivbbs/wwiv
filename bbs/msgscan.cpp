...
void scan(int nMessageNumber, int nScanOptionType, int *nextsub, bool bTitleScan) {
  ...
  do {
    ...
    CheckForHangup();
    set_net_num((session()->xsubs[session()->GetCurrentReadMessageArea()].nets.empty()) ? 0 :
                session()->xsubs[session()->GetCurrentReadMessageArea()].nets[0].net_num);
    ...
    if (yesno()) {
      ...
      if (p3) {
        ...
        for (int j = 0; j < nMsgToValidate; j++) {
          send_net_post(p3 + j, session()->subboards[session()->GetCurrentReadMessageArea()].filename,
  ...
  if (!quit && !express) {
    ...
    if (!session()->user()->IsRestrictionPost() &&
        ...
        (session()->GetEffectiveSl() >= session()->subboards[session()->GetCurrentReadMessageArea()].postsl)) {
      bout << "|#5Post on " << session()->subboards[session()->GetCurrentReadMessageArea()].name << "? ";
      irt[0] = '\0';
      irt_name[0] = '\0';
...
void SetupThreadRecordsBeforeScan() {
  ...
  for (unsigned short tempnum = 1; tempnum <= session()->GetNumMessagesInCurrentMessageArea(); tempnum++) { // was 0.
    ...
    string b;
    readfile(&(pPostRec1->msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b);
...
void HandleScanReadPrompt(int &nMessageNumber, int &nScanOptionType, int *nextsub, bool &bTitleScan, bool &done,
  ...
  if (nUserInput != 0 && nUserInput <= session()->GetNumMessagesInCurrentMessageArea() && nUserInput >= 1) {
    ...
    case 'X':
      if (lcs() && nMessageNumber > 0 && nMessageNumber <= session()->GetNumMessagesInCurrentMessageArea() &&
          session()->subboards[session()->GetCurrentReadMessageArea()].anony & anony_val_net &&
...
void HandleScanReadAutoReply(int &nMessageNumber, const char *pszUserInput, int &nScanOptionType) {
  ...
  if (get_post(nMessageNumber)->ownersys && !get_post(nMessageNumber)->owneruser) {
    grab_user_name(&(get_post(nMessageNumber)->msg), session()->subboards[session()->GetCurrentReadMessageArea()].filename);
  }
  grab_quotes(&(get_post(nMessageNumber)->msg), session()->subboards[session()->GetCurrentReadMessageArea()].filename);
  ...
  if (okfsed() && session()->user()->IsUseAutoQuote() && nMessageNumber > 0 &&
    ...
    readfile(&(get_post(nMessageNumber)->msg),
             (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b);
    if (pszUserInput[0] == '@') {
  ...
  if (pszUserInput[0] == 'O' && (so() || lcs())) {
    ...
    case '2': {
      if (nMessageNumber > 0 && nMessageNumber <= session()->GetNumMessagesInCurrentMessageArea()) {
        string b;
        readfile(&(get_post(nMessageNumber)->msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b);
        ...
        if (!fileExtract.Open(File::modeBinary | File::modeCreateFile | File::modeReadWrite)) {
          ...
          char szBuffer[255];
          sprintf(szBuffer, "ON: %s", session()->subboards[session()->GetCurrentReadMessageArea()].name);
...
void HandleScanReadFind(int &nMessageNumber, int &nScanOptionType) {
  ...
  while (nTempMsgNum != nMsgNumLimit && !abort && !hangup && !fnd) {
    ...
    string b;
    if (readfile(&(get_post(nTempMsgNum)->msg), session()->subboards[session()->GetCurrentReadMessageArea()].filename, &b)) {
      StringUpperCase(&b);
  ...
  while (!abort && !hangup && ++i <= nNumTitleLines) {
    ...
    if (session()->user()->GetScreenChars() >= 80) {
      ...
      if ((p3->anony & 0x0f) &&
        ...
        string b;
        if (readfile(&(p3->msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b)) {
...
void HandleMessageMove(int &nMessageNumber) {
  if ((lcs()) && (nMessageNumber > 0) && (nMessageNumber <= session()->GetNumMessagesInCurrentMessageArea())) {
    ...
    if (nTempSubNum != -1) {
      ...
      postrec p1 = p2;
      string b;
      readfile(&(p2.msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b);
      bout.nl();
      ...
      iscan(nTempSubNum);
      open_sub(true);
      p2.msg.storage_type = static_cast<unsigned char>(session()->subboards[session()->GetCurrentReadMessageArea()].storage_type);
      savefile(b, &(p2.msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename));
      ...
      if ((!(session()->subboards[session()->GetCurrentReadMessageArea()].anony & anony_val_net)) ||
          (session()->xsubs[session()->GetCurrentReadMessageArea()].nets.empty())) {
      ...
      if (!session()->xsubs[session()->GetCurrentReadMessageArea()].nets.empty()) {
        ...
        send_net_post(&p2, session()->subboards[session()->GetCurrentReadMessageArea()].filename,
                      session()->GetCurrentReadMessageArea());
      }
      add_post(&p2);
      close_sub();
...
void HandleMessageReply(int &nMessageNumber) {
  ...
  postrec p2 = *get_post(nMessageNumber);
  grab_quotes(&(p2.msg), session()->subboards[session()->GetCurrentReadMessageArea()].filename);
...
void HandleMessageExtract(int &nMessageNumber) {
  ...
  if ((nMessageNumber > 0) && (nMessageNumber <= session()->GetNumMessagesInCurrentMessageArea())) {
    string b;
    if (readfile(&(get_post(nMessageNumber)->msg), (session()->subboards[session()->GetCurrentReadMessageArea()].filename), &b)) {
