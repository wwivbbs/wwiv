...
static void show_chain(const chain_t& c, bool ansi, int chain_num, bool& abort) {
  ...
  if (ansi) {
    bout.bpla(
        fmt::format(" |#{:d}\xB3|#5{:3d}|#{:d}\xB3|#1{:>41.41}|#{:d}\xB3|{:2d}{:21.21}|#{:d}\xB3|#1{:5d}|#{:d}\xB3",
  ...
  for (const auto rb : r) {
    ...
    if (ansi) {
      bout.bpla(fmt::format(" |#{:d}\xB3   \xBA{:41.41}\xB3|#2{:21.21}|#{:d}\xB3{:5.5}\xB3", FRAME_COLOR, " ",
...
static void show_chains(int *mapp, std::map<int, int>& map) {
  ...
  if (a()->chains->HasRegisteredChains()) {
    ...
    if (a()->config()->toggles().show_chain_usage) {
      bout.bpla(fmt::format(" |#5Num |#1{:28.28}|#1{:5.5} |#5Num |#1{:28}|#1{:5}", "Description", "Usage", "Description", "Usage"), &abort);
      if (okansi()) {
    ...
    for (auto i = 0; i < *mapp && !abort && !a()->sess().hangup(); i++) {
      ...
      bout.outstr(fmt::format("{}|#2{:2d}{} |#1{:27.27}{}|#1{:5d}{}",
			       bar, i + 1,
	  ...
	  bout.bpla(fmt::format("|#2{:2d}{} |#1{:27.27}{}|#1{:5d}{}",
				 i + 1, bar,
    ...
    const auto bar = fmt::format("|#{:d}{}", FRAME_COLOR, okansi() ? "\xB3": "|") ; 
    for (auto i = 0; i < *mapp && !abort && !a()->sess().hangup(); i++) {
      bout.outstr(fmt::format("{}|#2{:2d}{} |#1{:33.33}{}", bar, i + 1, bar, a()->chains->at(map[i]).description, bar), &abort, &next);
      i++;
      if (!abort && !a()->sess().hangup()) {
        if (i >= *mapp) {
          bout.bpla(fmt::format("  {}                                  {}",bar,bar), &abort);
        } else {
          bout.bpla(fmt::format("|#2{:2d}{} |#1{:33.33}{}", i + 1, bar,
                                 a()->chains->at(map[i]).description, bar),
                    &abort);
        }
      }
