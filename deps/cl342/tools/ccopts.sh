...
if [ $ANALYSE -gt 0 ] ; then
	...
	CCARGS="$CCARGS -Wall -Waggregate-return -Wcast-align \
					...
					-Wredundant-decls -Wshadow -Wstrict-overflow \
					-Wstrict-prototypes -Wtype-limits -Wundef -Wvla" ;
	...
	if [ "$GCC_VER" -ge 53 ] ; then
		echo "  (Enabling additional compiler options for gcc 5.3.x)." >&2 ;
