...
FrameBuffer::FrameBuffer(int cols) : cols_(cols) { 
  b_.reserve(cols_ * 100);
}
...
bool FrameBuffer::write(char c, uint8_t a) { 
  ...
  case 0: // NOP
    break;
  case '\r': {
    auto line = pos_ / cols_;
    return gotoxy(0, line);
  } break;
  case '\n': {
    auto line = (pos_ / cols_) + 1;
    return gotoxy(0, line);
  } break;
  default: {
    return put(pos_++, c, a); 
...
void FrameBuffer::close() { 
  open_ = false; 
  b_.shrink_to_fit();
  for (auto i = std::rbegin(b_); i++ != std::rend(b_); i++) {
    if (i->c() != 0) {
      // Were at the end.
      auto rlast = std::distance(std::rbegin(b_), i);
      auto last = b_.size() - rlast;
      b_.resize(last);
      break;
    }
  }
...
bool FrameBuffer::grow(int pos) {
  ...
  auto current_line = current / cols_;

  b_.resize( (current_line + 1 + 25) * cols_);
  return true;
}
